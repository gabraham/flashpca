...
at https://github.com/gabraham/flashpca/data/DILGOM/rna_metab_data.R.

The preprocessed datasets can be downloaded via:
```{r load_data}
rna <- fread("https://github.com/gabraham/flashpca/data/DILGOM/rna.txt.gz")
metab <- fread("https://github.com/gabraham/flashpca/data/DILGOM/metab.txt.gz")
dim(rna)
dim(metab)
```
...
lambda2 <- seq(1e-5, 0.15, length=25)

tmp <- tempdir()
download.file("https://github.com/gabraham/flashpca/data/DILGOM/scca_cv.rda", tmpdir)
load(paste0(tmpdir, "/scca_cv.rda"))
if(!exists("s1")) {
...
```{r plot_scca}
rd <- data.table(r=as.numeric(s1$corr[1,,]),
   nz.x=as.numeric(s1$nzero.x[1,,]),
   nz.y=as.numeric(s1$nzero.y[1,,]),
   lambda1=rep(lambda1, length(lambda2)),
   lambda2=rep(lambda2, each=length(lambda1)))
g1 <- ggplot(rd, aes(x=lambda1, y=lambda2, fill=r))
g1 <- g1 + geom_raster()
g1 <- g1 + geom_contour(aes(z=r), bins=50, colour="white") + theme_bw()
g1 <- g1 + scale_fill_viridis("Correlation")
g1 <- g1 + scale_x_continuous(expression(lambda[1]))
g1 <- g1 + scale_y_continuous(expression(lambda[2]))
g1 <- g1 + geom_point(aes(x=s1.best.lambda1, y=s1.best.lambda2),
   data=data.frame(s1$best.lambda1, s1$best.lambda2),
...
We see that SCCA has identified about 60 genes (probes) associated with metabolite levels, and that the top genes identified indeed correspond to the genes described in the LL module.
