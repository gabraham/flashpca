...
#'
#' @param opt_dim Integer. Which dimension to select the optimal penalties
#' for.
#'
...
cv.scca <- function(X, Y,
   ...
   ndim=1, nfolds=10, folds=NULL, opt_dim=1, parallel=FALSE, init=TRUE,
   verbose=FALSE, return_models=FALSE, return_folds=FALSE, ...)
{
   n <- nrow(Y)
   if(is.character(X)) {
      stop(
   ...
   if(opt_dim <=0 || opt_dim > ndim) {
      stop("opt_dim must be between 1 and ndim")
   }
   ...
   r.mx <- max(r[opt_dim,,], na.rm=TRUE)
   w <- which(r[opt_dim,,] == r.mx, arr.ind=TRUE)
   ...
   res2 <- list(
      ...
      lambda2=lambda2,
      opt_dim=opt_dim,
      best_lambda1=lambda1[w[1]],
      best_lambda2=lambda2[w[2]],
      best_corr=r.mx,
      corr=r,
      nzero_x=nzx,
      nzero_y=nzy,
      nfolds=nfolds,
      converged=converged,
      models=if(return_models) {res} else {NULL},
      folds=if(return_folds) {folds} else {NULL}
   )
   class(res2) <- "cv.scca"
   res2
}
...
#'
#' @param keep_svd Logical. Whether to keep the singular value decomposition
#' results.
#' 
...
#' @export
#'
whiten <- function(X, ndim=20, keep_svd=TRUE, stand="none")
{
...
#'
#' @param test_prop numeric. What proportion of the data to reserve for
...
#' @export
#'
validate.rank <- function(X, maxdim=20, test_prop=0.1, const=0)
{
   X0 <- X1 <- X
   
   mx <- sample(length(X), size=test_prop * length(X))
