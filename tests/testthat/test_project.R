...
test_that("Testing projection", {
   ...
   pr2 <- project(bedf,
      loadings = f2$loadings, ref_allele = refallele,
...
test_that("Testing projection input checking", {
   ...
   expect_error(
      project(bedf,
         loadings = f$loadings, ref_allele = sample(refallele),
   ...
   expect_error(
      project(bedf,
         loadings = f$loadings[1:10], ref_allele = refallele,
         orig_mean = attr(X1, "scaled:center"),
         orig_sd = attr(X1, "scaled:scale")
      )
   )
