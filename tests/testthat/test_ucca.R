...
Bx <- matrix(stats::rnorm(m * p), m, p)
By <- matrix(stats::rnorm(m * k), m, k)
X0 <- scale(M %*% Bx + stats::rnorm(n * p))
Y0 <- scale(M %*% By + stats::rnorm(n * k))
...
test_that("Testing input checking", {
   ...
   Z <- matrix(stats::rnorm((nrow(X) + 3) * 10), nrow(X) + 3, 10)
   ...
   Z <- matrix(stats::rnorm(nrow(X) * (nrow(X) + 3)), nrow(X), nrow(X) + 3)
   expect_error(
      ucca(X, Z, standx = "none", standy = "none", mem = "high")
   )
   expect_error(
