...
#' @export
project <- function(
   X, loadings, orig_mean = NULL, orig_sd = NULL,
   ...
   if (is.null(orig_mean)) {
      stop(
         "The vector of means used for standardising",
         " the data must be provided via 'orig_mean'"
      )
   } else if (is.null(orig_sd)) {
      stop(
         "The vector of standard deviations used for standardising",
